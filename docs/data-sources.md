...
約914MBのCityGML全体は現行Web配信に使いません。公式3D Tiles ZIPと展開物もraw領域に保ち、Gitへcommitしません。
...
静的配信用には、東舞鶴駅・西舞鶴駅から100m以内のleaf content regionを持つ5 b3dmを選択しました。

| Subset property | Verified value |
|---|---:|
| b3dm payload | 12,723,708 B |
| current subset including metadata | 12,729,687 B |
| unique buildings | 2,152 |
| actual LOD2 | 937（43.541%） |
| actual LOD1 | 1,215（56.459%） |
| usage known excluding `不明` | 1,903（88.429%） |
| measured height known | 1,738（80.762%） |
| above/below-ground storeys known | 1,903（88.429%） |
| footprint/total floor area known | 1,903（88.429%） |
...
ソフトウェアlicenseとデータlicenseは別です。repositoryには現時点でソフトウェアの `LICENSE` ファイルがないため、コードの再利用条件は権利者へ確認してください。
