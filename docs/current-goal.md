...
The earlier three corrections remain intact. Both residual checks and all related local regressions pass on source `8060f3f0c6205cc1b7f6ce75a2b9607cae72519a`: metadata/relation separation is +2/+3 CSS px with unchanged fonts; Guided 1440 split/strict-3D/credits/keyboard checks pass with ordered readiness and one shared finite deadline. The historical 5-second failure was not reproduced; a separate observed DOM/CSS SVG-transition mismatch was resolved by measuring after actual paint, not by weakening assertions or changing layout. Public/Guided/Advanced at four widths, actual browser 200%, loading/retry, identity/native regressions, lint/typecheck/225 tests/build/diff pass on the same build. Exactly two review images were captured and inspected; old evidence is preserved. Normal feature push, exact final-head CI 9/9, Pages and production verification must finish before marking the existing goal complete. Do not deploy if any required final check fails. Stop after these two issues and publication verification; no broader design or media task follows.
