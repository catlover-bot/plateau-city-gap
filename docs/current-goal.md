...
Active goal: `final-product-uiux-polish-v1`, resumed with explicit two-issue authorization from remote feature HEAD `035f295e21e3b5f47e4e9e363b7d4d969861f476`, not restarted. The [short final UI/UX checkpoint](final-uiux-checkpoint.md) records the minimal callout-spacing change and render/layout-aware test sequencing. Adopted Guided 3D media and all previous presentation assets/provenance/backups are read-only. Preserve the existing two image deletions and two untracked copies. No new recording, palette, feature, repository cleanup or main promotion.

**Not complete: production fresh-entry 3D verification remains unresolved.** Local corrections, four-width/actual-200% UI regressions, 225 tests, lint/typecheck/build and two review images passed on application source `8060f3f0c6205cc1b7f6ce75a2b9607cae72519a`. Evidence HEAD `76b1f0c40e23ee73f0f3b64e5319e30b87af89da` then passed exact-head CI 9/9 and Pages; live entry/all ten JS/CSS hashes match. After deployment, software Guided initial 3D hit the real 45-second render-error UI, and native Advanced's first fresh child-page entry failed strict readiness. A 23-second instrumented success does not erase either failure. Their original evidence and failure image are preserved by the checkpoint. The 1px detail overlap is corrected; the original Section layout synchronization fix passes locally, but overall production startup reliability is not accepted. No further deployment, timeout relaxation, blind retry, cleanup, media work or broader design change is authorized by a green CI result. Stop with these concrete remaining checks; do not mark the existing goal complete.
