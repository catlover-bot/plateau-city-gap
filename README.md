...
公開成果と一致させる事実・主張境界は次のとおりです。

- 人口分析の範囲は舞鶴市495メッシュと藤沢市327メッシュです。
- PLATEAU建物は全市配信44,640建物の分布を示しますが、公開する属性付き参照層は検証済み856棟subset、そのうち深掘り範囲は対象メッシュ内296棟です。Top 10メッシュの公式建物coverageは0棟であり、欠損を推測で補いません。
- 建物への人口表示はモデル推計配分であり、実居住者数ではありません。
- 道路面の関係はexperimental PLATEAU LOD1 road-surface adjacencyです。徒歩時間として扱いません。
- 標高は実DEMの断面表示に限り、DEMから歩行負荷・危険度・斜度を推定しません。
